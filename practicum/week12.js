...
const makeTwenty = () => {
	//Ваш код
	resultTwenty.innerHTML = `максимальное число из чисел 10, 25, 5, 30, 15 это ${Math.max(10, 25, 5, 30, 15)}`;
};
...
const makeTwentyOne = () => {
	//Ваш код
	resultTwentyOne.innerHTML = Math.round(Math.random() * 11);
};
...
const makeTwentyFour = () => {
	//Ваш код
	resultTwentyFour.textContent = `наименьшее число это ${Math.min(7, 3, 11, 5, 9)}`;
};
...
const makeTwentyFive = () => {
	//Ваш код
	resultTwentyFive.innerHTML = `А здесь ответ ${Math.max(7, 3, 11, 5, 9)}`;
};
